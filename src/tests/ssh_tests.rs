...
mod ssh_tests {
    ...
    fn test_ssh_path_parser() {
        ...
        assert_eq!(location.host, "localhost");
        assert_eq!(location.path, PathBuf::from("/tmp"));
        assert_eq!(location.ssh_options, vec!["-v", "-o", "StrictHostKeyChecking=no"]);
        assert!(location.rsync_options.is_empty());
    ...
    fn test_ssh_command_generation() {
        ...
        let location = RemoteLocation::parse("ssh:localhost:/tmp:-v,-o,StrictHostKeyChecking=no:").unwrap();
        let cmd = location.ssh_command();
        assert_eq!(cmd, vec!["ssh", "-v", "-o", "StrictHostKeyChecking=no", "localhost"]);
    }
    ...
    fn test_rsync_command_generation() {
        ...
        assert_eq!(cmd, vec!["rsync", "-avz", "/local/path", "localhost:/remote/path"]);
        ...
        let cmd = location.rsync_command(&source, &dest, false);
        assert_eq!(cmd, vec!["rsync", "-avz", "-e", "ssh -p 2222", "/local/path", "localhost:/remote/path"]);

        // With SSH options
        let location = RemoteLocation::parse("ssh:localhost:/remote/path:-v:").unwrap();
        let cmd = location.rsync_command(&source, &dest, false);
        assert_eq!(cmd, vec!["rsync", "-avz", "-e", "ssh -v", "/local/path", "localhost:/remote/path"]);

        // With Rsync options
        let location = RemoteLocation::parse("ssh:localhost:/remote/path::--progress").unwrap();
        let cmd = location.rsync_command(&source, &dest, false);
        assert_eq!(cmd, vec!["rsync", "-avz", "--progress", "/local/path", "localhost:/remote/path"]);
    }
