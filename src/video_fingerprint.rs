...
pub fn fingerprint_video(path: &Path) -> Result<Vec<u8>> {
    if !crate::media_dedup::is_ffmpeg_available() {
        return Err(anyhow::anyhow!("ffmpeg is not available for video fingerprinting"));
    }
    ...
    if keyframes.is_empty() {
        return Err(anyhow::anyhow!("Could not extract any keyframes from video"));
    }
    
    // Generate perceptual hash for each keyframe and combine them
    let mut fingerprint = Vec::new();
    let hasher = HasherConfig::new().hash_alg(HashAlg::DoubleGradient).hash_size(8, 8).to_hasher();
...
fn extract_keyframes(path: &Path) -> Result<Vec<DynamicImage>> {
    ...
    let output = Command::new("ffmpeg")
        .args(&[
            ...
            "-vf", "select=eq(pict_type\\,I)",  // Only extract I-frames
            "-vsync", "vfr",                   // Variable framerate output
            "-qscale:v", "2",                  // High quality
            "-frames:v", "5",                  // Limit to 5 keyframes max
    ...
    for i in 1..=5 {  // Try all 5 potential frames
...
pub fn extract_video_metadata(path: &Path) -> Result<(Option<u32>, Option<u32>, Option<f64>, Option<u32>)> {
    if !crate::media_dedup::is_ffmpeg_available() {
        return Err(anyhow::anyhow!("ffmpeg is not available for video metadata extraction"));
    }
    ...
    let bitrate = json["streams"][0]["bit_rate"].as_str()
