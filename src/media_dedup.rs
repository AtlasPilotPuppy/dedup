...
use exif::{Reader as ExifReader, In, Tag};
use log;
use hex;
...
pub fn extract_image_metadata(path: &Path) -> Result<MediaMetadata> {
    ...
    let img_hash_img = {
        ...
        let buffer = img_hash::image::ImageBuffer::from_raw(
...
pub fn extract_video_metadata(path: &Path) -> Result<MediaMetadata> {
    ...
    if !is_ffmpeg_available() {
        return Err(anyhow::anyhow!("ffmpeg is required for video processing but is not available"));
    }
...
pub fn extract_audio_metadata(path: &Path) -> Result<MediaMetadata> {
    ...
    if !is_ffmpeg_available() {
        ...
        if !chromaprint_available {
            return Err(anyhow::anyhow!("Neither ffmpeg nor chromaprint is available for audio processing"));
        }
    }
        ...
        if let Ok(output) = output {
            if output.status.success() {
                ...
                if let Ok(json) = serde_json::from_str::<serde_json::Value>(&output_str) {
                    // Extract duration
                    duration = json["streams"][0]["duration"].as_str()
                        .and_then(|d| d.parse::<f64>().ok())
                        .or_else(|| json["streams"][0]["duration"].as_f64());
                    
                    // Extract bitrate
                    bitrate = json["streams"][0]["bit_rate"].as_str()
...
pub fn compare_media_files(a: &MediaFileInfo, b: &MediaFileInfo) -> u32 {
    match (&a.metadata, &b.metadata) {
        (Some(meta_a), Some(meta_b)) => {
            ...
            match meta_a.kind {
                MediaKind::Image => {
                    match (&meta_a.perceptual_hash, &meta_b.perceptual_hash) {
                        (Some(hash_a), Some(hash_b)) => calculate_image_similarity(hash_a, hash_b),
                        _ => 0,
                    }
                }
                MediaKind::Video => {
                    match (&meta_a.fingerprint, &meta_b.fingerprint) {
...
pub fn determine_preferred_media_file<'a>(
    ...
    let files_with_metadata: Vec<_> = files.iter()
        .filter(|f| f.metadata.is_some())
        .collect();
    ...
    let format_ranks: HashMap<String, usize> = options.format_preference.formats.iter()
        .enumerate()
    ...
    let best_format_files: Vec<_> = preferred_format_files.into_iter()
        ...
        match options.resolution_preference {
            ...
            ResolutionPreference::Lowest => {
                ...
                best_format_files.into_iter()
                    .min_by_key(|file| {
                        match &file.metadata {
                            Some(meta) => meta.width.unwrap_or(u32::MAX) * meta.height.unwrap_or(u32::MAX),
                            None => u32::MAX,
                        }
                    })
            }
                    ...
                    .min_by_key(|file| {
                        match &file.metadata {
                            Some(meta) => {
                                ...
                                let dw = if w > target_width { w - target_width } else { target_width - w };
                                let dh = if h > target_height { h - target_height } else { target_height - h };
...
pub fn find_similar_media_files(
    ...
    log::info!("Starting media deduplication with threshold: {}%", options.similarity_threshold);
    ...
    let media_files: Vec<MediaFileInfo> = file_infos.iter()
        .map(|file_info| {
            ...
            if media_kind != MediaKind::Unknown {
                media_file.metadata = match extract_media_metadata(&file_info.path) {
                    ...
                    Err(e) => {
                        log::warn!("Failed to extract media metadata for {:?}: {}", file_info.path, e);
                        None
                    }
                };
            }
    ...
    log::info!("Media file count: {} images, {} videos, {} audio files", 
    ...
    log::info!("Found {} groups of similar media files.", similar_groups.len());
...
pub fn convert_to_duplicate_sets(
    ...
    for group in similar_groups {
        ...
        if let Some(kept) = kept_file {
            ...
            let mut file_infos = group.iter()
                .map(|mf| mf.file_info.clone())
                .collect::<Vec<_>>();
            
            // Ensure the kept file is first (for UI presentation)
            if let Some(kept_idx) = file_infos.iter().position(|f| f.path == kept.file_info.path) {
            ...
            let hash = format!("media_{}", group[0].file_info.path.to_string_lossy().to_string());
...
pub fn add_media_options_to_cli(options: &mut MediaDedupOptions, enable: bool, resolution: &str, formats: &[String], threshold: u32) {
...
mod tests {
    ...
    fn test_format_preference() {
        ...
        assert!(format_pref.formats.iter().position(|f| f == "raw").unwrap() < 
                format_pref.formats.iter().position(|f| f == "jpg").unwrap());
                
        // Check if png is higher priority than jpg (lossless over lossy)
        assert!(format_pref.formats.iter().position(|f| f == "png").unwrap() < 
                format_pref.formats.iter().position(|f| f == "jpg").unwrap());
    }
    ...
    fn test_resolution_preference_display() {
        ...
        assert_eq!(ResolutionPreference::ClosestTo(1280, 720).to_string(), "closest to 1280x720");
    }
