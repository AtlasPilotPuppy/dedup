...
impl FileCache {
    ...
    pub fn new(cache_dir: &Path, algorithm: &str) -> Result<Self> {
        ...
        if cache_file.exists() {
            ...
            log::info!("Loaded {} entries from cache file: {:?}", entries.len(), cache_file);
        }
    ...
    pub fn store(&mut self, file_info: &FileInfo, algorithm: &str) -> Result<()> {
        if let Some(entry) = FileCacheEntry::from_file_info(file_info, algorithm) {
            ...
            Err(anyhow::anyhow!("Cannot cache file without hash: {:?}", file_info.path))
        }
    }
    ...
    pub fn save(&mut self) -> Result<()> {
        ...
        fs::rename(&temp_file, &cache_file)
            .with_context(|| format!("Failed to rename temp cache file: {:?} to {:?}", temp_file, cache_file))?;
            
        log::info!("Saved {} entries to cache file: {:?}", self.entries.len(), cache_file);
