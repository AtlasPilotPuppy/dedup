...
impl DedupClient {
    ...
    pub fn connect(&mut self) -> Result<()> {
        ...
        let stream = TcpStream::connect(format!("{}:{}", self.host, self.port))
            .with_context(|| format!("Failed to connect to dedups server at {}:{}", self.host, self.port))?;
        ...
        let mut reader_protocol = protocol.stream.try_clone()
        ...
        let reader_thread = thread::spawn(move || {
            ...
            loop {
                buf.clear();
                let bytes_read = match std::io::BufRead::read_until(&mut buf_reader, b'\n', &mut buf) {
                    Ok(0) => break, // EOF
                    Ok(n) => n,
                    Err(e) => {
    ...
    pub fn send_command(&mut self, command: String, args: Vec<String>, options: HashMap<String, String>) -> Result<()> {
    ...
    pub fn execute_command(&mut self, command: String, args: Vec<String>, options: HashMap<String, String>) -> Result<String> {
                ...
                Some(msg) => {
                    match msg.message_type {
                        ...
                        MessageType::Error => {
                            ...
                            has_error = true;
                            output.push_str(&format!("Error (code {}): {}", error.code, error.message));
                            break;
                        }
