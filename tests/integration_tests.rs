    ...
    pub fn new() -> Self {
        ...
        if root_path.exists() {
            fs::remove_dir_all(&root_path).unwrap_or_else(|e| 
                panic!("Failed to clean up existing test directory {:?}: {}", root_path, e)
            );
        }
        fs::create_dir_all(&root_path).unwrap_or_else(|e| 
            panic!("Failed to create test directory {:?}: {}", root_path, e)
        );

        let mut env = Self { root_path, rng };
        env.create_test_files().unwrap_or_else(|e| 
            panic!("Failed to create test files in new TestEnv: {}", e)
        );
        env
    }
    ...
    fn create_test_files(&mut self) -> Result<()> {
        ...
        for i in 0..NUM_SUBFOLDERS {
            ...
            for j in 0..FILES_PER_SUBFOLDER {
                ...
                let content_index = (i * FILES_PER_SUBFOLDER + j) % (NUM_DUPLICATE_CONTENT_SETS + 1);
                ...
                let content_to_write = if content_index < NUM_DUPLICATE_CONTENT_SETS {
                    ...
                    if *current_count < MAX_DUPLICATES_PER_SET {
                        *current_count += 1;
                        ...
                        let max_len = (FILE_SIZE_MAX - UNIQUE_CONTENT_PREFIX.len() - 5).max(FILE_SIZE_MIN);
                        let len = self.rng.gen_range(FILE_SIZE_MIN..=max_len);
                        let random_part = self.generate_random_string(len);
...
mod integration {
    ...
    fn test_environment_setup_cleanup() -> Result<()> {
        let env = TestEnv::new();
        assert!(env.path().exists(), "Test directory should exist after setup.");
        ...
        assert_eq!(found_folders, NUM_SUBFOLDERS, "Incorrect number of subfolders created.");
        assert_eq!(found_files, NUM_SUBFOLDERS * FILES_PER_SUBFOLDER, "Incorrect number of files created.");

        env.cleanup()?;
        assert!(!env.path().exists(), "Test directory should not exist after cleanup.");
        Ok(())
    }
    ...
    fn setup_basic_duplicates(env: &mut TestEnv) {
        ...
        env.create_file_with_content_and_time(&subdir1.join("fileA.txt"), "contentA", Some(now - Duration::from_secs(3600)));
        env.create_file_with_content_and_time(&subdir1.join("fileB.txt"), "contentB", Some(now - Duration::from_secs(7200)));
        env.create_file_with_content_and_time(&subdir2.join("fileC.txt"), "contentA", Some(now)); // Duplicate of fileA.txt
        env.create_file_with_content_and_time(&subdir2.join("fileD.txt"), "contentD", Some(now - Duration::from_secs(100)));
        // A deeply nested duplicate
        let deep_subdir = env.create_subdir("sub2/deep");
        env.create_file_with_content_and_time(&deep_subdir.join("fileE.txt"), "contentB", Some(now - Duration::from_secs(300))); // Duplicate of fileB.txt
    }
    ...
    fn test_find_duplicates_integration() -> Result<()> {
        ...
        env.create_file_with_content_and_time(&env.root().join("unique.txt"), "unique_content", None);
        ...
        for set in &duplicate_sets {
            if set.files.len() >= MIN_DUPLICATES_PER_SET {
                ...
                for file_info in &set.files {
                    assert_eq!(file_info.hash.as_ref().expect("File should have a hash"), first_hash);
    ...
    fn test_delete_files_integration() -> Result<()> {
        ...
        if files_to_delete_info.is_empty() { // If truly no files to delete, the test might not be meaningful
        ...
        assert_eq!(delete_count, files_to_be_deleted_paths.len(), "Mismatch in number of deleted files.");

        // Verify files were deleted and kept files still exist
        for path in files_to_be_deleted_paths {
            assert!(!path.exists(), "File {:?} should have been deleted but still exists.", path);
        }
        for path in files_to_be_kept_paths {
            assert!(path.exists(), "File {:?} should have been kept but was deleted.", path);
        }
    ...
    fn test_move_files_integration() -> Result<()> {
        ...
        for original_path in &files_to_be_moved_original_paths {
            assert!(!original_path.exists(), "File {:?} should have been moved from original location.", original_path);
            ...
            for entry in fs::read_dir(&target_move_dir)? {
                ...
                if entry.path().is_file() {
                    if entry.path().file_name().unwrap_or_default().to_string_lossy().starts_with(&*original_path.file_stem().unwrap_or_default().to_string_lossy()) {
                        moved_correctly_count += 1;
                        *found_map.entry(original_path.clone()).or_insert(0) += 1;
                    }
                }
            }
            assert_eq!(moved_correctly_count, 1, "Expected exactly one file to be moved correctly.");
            assert_eq!(found_map.len(), 1, "Expected exactly one original file to be found in the target directory.");
        }
        for path in files_to_be_kept_paths {
            assert!(path.exists(), "File {:?} should have been kept but was moved/deleted.", path);
        }
        Ok(())
    }
    ...
    fn test_output_duplicates_integration() -> Result<()> {
        ...
        if actionable_duplicate_sets_count > 0 { 
            ...
            let parsed_json: Result<HashMap<String, serde_json::Value>, _> = serde_json::from_str(&json_content);
            assert!(parsed_json.is_ok(), "Failed to parse output JSON: {:?}", parsed_json.err());
            if let Ok(map) = parsed_json {
                assert_eq!(map.len(), actionable_duplicate_sets_count, "Mismatch in number of sets in JSON output.");
            }
        } else {
            // If no actionable duplicates, output_duplicates should not create a file.
            assert!(!json_output_path.exists(), "JSON output file was created unexpectedly for empty actionable duplicates.");
        }
        ...
        if actionable_duplicate_sets_count > 0 {
            assert!(toml_output_path.exists(), "TOML output file was not created.");
            let toml_content = fs::read_to_string(&toml_output_path)?;
            assert!(!toml_content.is_empty(), "TOML output file is empty.");
            let parsed_toml: Result<HashMap<String, toml::Value>, _> = toml::from_str(&toml_content);
            assert!(parsed_toml.is_ok(), "Failed to parse output TOML: {:?}", parsed_toml.err());
             if let Ok(map) = parsed_toml {
                assert_eq!(map.len(), actionable_duplicate_sets_count, "Mismatch in number of sets in TOML output.");
            }
        } else {
