...
fn test_ssh_remote_basic_operations() -> Result<()> {
    ...
    let write_test =
        remote.run_command("touch /tmp/dedups_test_file && echo 'success' || echo 'failed'")?;
    assert!(
        write_test.contains("success"),
        "Failed to write to /tmp on remote host"
    );
...
fn test_is_remote_path() -> Result<()> {
    // Test path detection
    assert!(file_utils::is_remote_path(Path::new(
        "ssh:testuser@localhost:2222:/tmp"
    )));
...
fn test_check_dedups_installed() -> Result<()> {
    ...
    println!(
        "Is dedups installed on Docker test container: {:?}",
        is_installed
    );
...
fn test_file_operations() -> Result<()> {
    ...
    if RemoteLocation::parse("ssh:testuser@localhost:2222:/tmp")
        .and_then(|r| ensure_ssh_access(&r))
        .is_err()
    {
        return Ok(());
    }
    ...
    assert!(
        check_result.contains("exists"),
        "Remote file was not created"
    );
...
fn test_handle_directory() -> Result<()> {
    ...
    let mut cli = Cli::parse_from([
        "dedups",
        "ssh:testuser@localhost:2222:/home/testuser/test_data",
    ]);
    ...
    let files = file_utils::handle_directory(
        &cli,
        Path::new("ssh:testuser@localhost:2222:/home/testuser/test_data"),
    )?;
...
EOF
    ...
    assert!(
        !output.is_empty(),
        "dedups --version should return version info"
    );

    // Try to scan a directory
    let scan_output = protocol.execute_dedups(&["/tmp", "--dry-run"], &cli)?;
    assert!(
        !scan_output.is_empty(),
        "dedups scan should return some output"
    );
...
EOF
    ...
    assert!(
        dedups_path.is_some(),
        "dedups should be found in ~/.local/bin"
    );
    ...
    let output = protocol.execute_dedups(&["--version"], &cli)?;
    assert!(
        output.contains("dedups v0.1.0-test"),
        "Should execute the test dedups script"
    );
